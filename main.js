...
// 🔹 Función global de validación de owners
global.isOwner = (jid = "") => {
  const num = jid.replace(/\D/g, ""); // extrae solo números
...
module.exports = async (client, m) => {
  ...
  const isAdmins = m.isGroup
    ? resolvedAdmins.some((p) => p.jid === sender)
    : false;
  ...
  if (global.comandos.has(command)) {
    ...
    // ✅ Validación robusta de owners
    if (cmdData.isOwner && !global.isOwner(sender))
      return m.reply(mess.owner);

    if (cmdData.isReg && !db.data.users[sender]?.registered)
